        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/user/toAddPage'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
                    <br>
                    <hr style="clear:both;">
...
<script type="text/javascript">
    ...
    function toEditPage(uid) {
        window.location.href='${APP_PATH}/user/toEditPage?id='+uid;
    }

    function deleteUsers() {
       var userData=$("#userIds:checked")
        if (userData.length==0){
            layer.msg("请选择你要删除的用户", {time:1000, icon:0, shift:6}, function () {});
            return;
        }
        layer.confirm("是否要删除选中用户",  {icon: 3, title:'提示'}, function(cindex){
            //确认回调函数
            $.ajax({
    ...
    function toAssignPage(uid) {
        window.location.href="${APP_PATH}/role/toAssignPage?uid="+uid;
    }
    function deleteUser(id,loginAccount) {
        ...
        layer.confirm("是否要删除用户:"+loginAccount,  {icon: 3, title:'提示'}, function(cindex){
            //确认回调函数
            $.ajax({
                ...
                if (result.success){
                   ...
                   var navigateInfo="";
                    $.each(result.data.list,function (i,user) {
                        ...
                        tbodyContent+="  <td><input type='checkbox' onclick='checkIsSelectedAll("+result.data.pageSize+")' id='userIds' name='userIds' value='"+user.id+"'></td>"
                        tbodyContent+="     <td>"+user.loginAccount+"</td>"
                        tbodyContent+="      <td>"+user.username+"</td>"
                        tbodyContent+="      <td>"+user.email+"</td>"
                        tbodyContent+="      <td>"
                        tbodyContent+="      <button type='button' class='btn btn-success btn-xs' onclick='toAssignPage("+user.id+")'><i class=' glyphicon glyphicon-check'></i></button>"
                        tbodyContent+="  <button type='button' class='btn btn-primary btn-xs' onclick='toEditPage("+user.id+")'><i class=' glyphicon glyphicon-pencil'></i></button>"
                        tbodyContent+="  <button type='button' class='btn btn-danger btn-xs' onclick=\"deleteUser('"+user.id+"','"+user.loginAccount+"')\"><i class='glyphicon glyphicon-remove'></i></button>"
                        tbodyContent+="  </td>"
                        tbodyContent+="  </tr>"
